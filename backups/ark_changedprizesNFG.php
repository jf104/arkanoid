...
//fix when many prize boxes drop
...
function checkIfPrizeHitBar(prizeBlockX)
{
	barX1 = barX;
...
{
	//repeating thins in ball hit block function can create new function to return this eventually
	X1_ball = ball.x;
...
//blah
	for(var x=0; x<blocks.length; x++)
	{
		...
		if(!block.hit && !block.prize)
		{
			...
			prizeGenerator(block);
				prizeBlockImg = choosePrizeBlockImage(block);
				drawPrizeBlock(prizeBlockImg);
	}
...
function drawPrizeBlock(prizeBlockImg)
{
	if(prizeBlockImg != 'no prize')
	{
		ark.drawImage(prizeBlockImg, currentBlock.x, currentBlock.y + PrizeBlockY);
		currentBlock.prizeCheck = true;
	}
}

function didPrizeHitBottom()
{
	if(PrizeBlockY > screenHeight - ballImg.height)
	{
		currentBlock.prizeCheck = false;
		PrizeBlockY = 0;
		return;
...
function prizeGenerator(currentBlock)
{
	if(currentBlock.prizeCheck)
	{
		if(!currentBlock.prize)
		{
			currentBlock.prize = Math.floor((Math.random()*2)+1);
		}
